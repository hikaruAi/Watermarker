...
def blend_watermark(original: Image, watermark: Image, scale_multiplier) -> Image:
    copy = original.copy()
    rotated = watermark.rotate(scale_multiplier, resample=Image.Resampling.BICUBIC, expand=True)
    rotated.save("rotated.png", "PNG")
    pad = copy.size[0] * 2 / scale_multiplier
...
def make_watermark(file_name: str, text: str, options: WatermarkConfig) -> Image:
    ...
    final = blend_watermark(input_img, blank_watermark, options.WATERMARK_SCALE_IMAGE)
